...
async def get_train_metadata(store: obstore.store.S3Store, sink_root: pathlib.Path) -> pd.DataFrame:
...
async def get_crosswalk(store: obstore.store.S3Store, sink_root: pathlib.Path) -> pd.DataFrame:
    dst = sink_root / "chip_id_crosswalk.parquet"
    if not dst.exists():
        r = await store.get_async("cloud-cover-detection-challenge/final/chip_id_crosswalk.csv")
        data = await r.bytes_async()
        df = pd.read_csv(io.BytesIO(data))
        df.to_parquet(dst)
        return df
...
async def download_chip(store: obstore.store.S3Store, sink_root: pathlib.Path, chip_id: str) -> None:
...
async def download_labels(store: obstore.store.S3Store, sink_root: pathlib.Path, chip_ids: str) -> None:
    for chip_id in chip_ids:
        await download_chip_labels(store, sink_root, chip_id)


async def download_chip_labels(store: obstore.store.S3Store, sink_root: pathlib.Path, chip_id: str) -> None:
...
async def download_chips(store: obstore.store.S3Store, sink_root: pathlib.Path, chip_ids: list[str]) -> None:
